...
public class RateBeerApi implements CommandService {
	...
	private static final String RB_KEY = "tTmwRTWT-W7tpBhtL";
	...
	public CommandResult execute(Command command) {
		try {
			...
			case GetUserDetails:
				...
				return parseUserDetails(getUserDetailsCommand, HttpHelper.makeRBGet("http://www.ratebeer.com/user/"
						+ getUserDetailsCommand.getUserId() + "/"));

			case GetUserRatings:
				GetUserRatingsCommand getUserRatingsCommand = (GetUserRatingsCommand) command;
				return parseUserRatings(getUserRatingsCommand, HttpHelper.makeRBGet("http://www.ratebeer.com/user/"
						+ getUserRatingsCommand.getForUserId() + "/ratings/" + getUserRatingsCommand.getPageNr() + "/"
						+ getUserRatingsCommand.getSortOrder() + "/"));

			case GetUserRating:
				ensureLogin(userSettings);
				GetUserRatingCommand getUserRatingCommand = (GetUserRatingCommand) command;
				return parseUserRating(getUserRatingCommand, HttpHelper.makeRBGet("http://www.ratebeer.com/beer/rate/"
						+ getUserRatingCommand.getBeerId() + "/"));

			case GetUserCellar:
				GetUserCellarCommand getUserCellarCommand = (GetUserCellarCommand) command;
				return parseUserCellar(getUserCellarCommand, HttpHelper.makeRBGet("http://www.ratebeer.com/user/"
						+ getUserCellarCommand.getForUserId() + "/cellar/"));

			case GetUserImage:
				GetUserImageCommand getUserImageCommand = (GetUserImageCommand) command;
				return parseUserImage(getUserImageCommand, HttpHelper.makeRawRBGet("http://www.ratebeer.com/UserPics/"
						+ getUserImageCommand.getUsername() + ".jpg"));

			case Search:
				SearchCommand searchCommand = (SearchCommand) command;
				return parseSearchResults(searchCommand, HttpHelper
						.makeRBGet("http://www.ratebeer.com/json/s.asp?k="
								+ RB_KEY
								+ "&b="
								+ URLEncoder.encode(searchCommand.getQuery(), UTF8)
								+ (searchCommand.getUserId() != SearchCommand.NO_USER ? "&u="
										+ searchCommand.getUserId() : "")));

			case GetBeerDetails:
				GetBeerDetailsCommand getBeerDetailsCommand = (GetBeerDetailsCommand) command;
				return parseBeerDetails(getBeerDetailsCommand, HttpHelper
						.makeRBGet("http://www.ratebeer.com/json/bff.asp?k=" + RB_KEY + "&bd="
								+ getBeerDetailsCommand.getBeerId()));

			case GetBeerImage:
				GetBeerImageCommand getBeerImageCommand = (GetBeerImageCommand) command;
				return parseBeerImage(getBeerImageCommand, HttpHelper
						.makeRawRBGet("http://www.ratebeer.com/beerimages/" + getBeerImageCommand.getBeerId() + ".jpg"));

			case GetBeerRatings:
				GetRatingsCommand getBeerRatingsCommand = (GetRatingsCommand) command;
				return parseRatings(getBeerRatingsCommand, HttpHelper
						.makeRBGet("http://www.ratebeer.com/json/gr.asp?k=" + RB_KEY + "&bid="
			...
			case GetStyleDetails:
				...
				return parseStyleBeers(getStyleDetailsCommand, HttpHelper
						.makeRBGet("http://www.ratebeer.com/beerstyles/s/" + getStyleDetailsCommand.getStyleId() + "/"));
			...
			case GetTopBeers:
				...
				case Top50:
					...
					return parseTop50Beers(getTopBeersCommand, HttpHelper
							.makeRBGet("http://www.ratebeer.com/beer/top-50/"));
				case TopByCountry:
					// TODO: Replace with API call http://www.ratebeer.com/json/tb.asp?m=country&c=<COUNTRYID>&k=<KEY>
					return parseTopByCountryBeers(getTopBeersCommand, HttpHelper
							.makeRBGet("http://www.ratebeer.com/beer/country/"
			...
			case GetPlaces:
				...
				return parsePlaces(getPlacesCommand, HttpHelper.makeRBGet("http://ratebeer.com/json/beerme.asp?k="
						+ RB_KEY + "&mi=" + getPlacesCommand.getRadius() + "&la=" + getPlacesCommand.getLatitude()
						+ "&lo=" + getPlacesCommand.getLongitude()));
			...
			case GetEvents:
				...
				return parseEvents(getEventsCommand, HttpHelper
						.makeRBGet("http://www.ratebeer.com/FestsInMyArea.asp?CountryID="
			...
			case GetEventDetails:
				...
				return parseEventDetails(getEventDetailsCommand, HttpHelper
						.makeRBGet("http://www.ratebeer.com/Events-Detail.asp?EventID="
			...
			case SetEventAttendance:
				...
				HttpHelper.makeRBPost("http://www.ratebeer.com/eventprocess-attend.asp", Arrays
						.asList(new BasicNameValuePair("EventID", Integer.toString(setEventAttendanceCommand.getEventId())),
			...
			case SetDrinkingStatus:
				...
				HttpHelper.makeRBPost("http://www.ratebeer.com/userstatus-process.asp", Arrays
						.asList(new BasicNameValuePair("MyStatus", setDrinkingStatusCommand.getNewStatus())),
				// Note that we get an HTTP 500 response even when the request is successfull...
						HttpStatus.SC_INTERNAL_SERVER_ERROR);
			...
			case PostRating:
				...
				// NOTE: Maybe use the API call to http://www.ratebeer.com/m/m_saverating.asp, but it should be checked whether this allows updating of a rating too
				HttpHelper.makeRBPost(postRatingCommand.getRatingID() <= 0 ? 
						"http://www.ratebeer.com/saverating.asp": 
						"http://www.ratebeer.com/updaterating.asp", 
					Arrays.asList(
						new BasicNameValuePair("BeerID", Integer.toString(postRatingCommand.getBeerId())), 
						new BasicNameValuePair("RatingID", postRatingCommand.getRatingID() <= 0 ? 
							"" : 
							Integer.toString(postRatingCommand.getRatingID())),
			...
			case GetFavouritePlaces:
				...
				return parseFavouritePlaces(getFavPlacesCommand, HttpHelper
						.makeRBGet("http://www.ratebeer.com/beer/availability-add/" + getFavPlacesCommand.getBeerId()
								+ "/"));

			case FindPlaces:
				ensureLogin(userSettings);
				// TODO: Replace with API call http://www.ratebeer.com/json/psstring.asp?s=b<SEARCHSTRING>&k=<KEY>
				FindPlacesCommand findPlacesCommand = (FindPlacesCommand) command;
				return parseFindPlaceResults(findPlacesCommand, HttpHelper.makeRBPost(
						"http://www.ratebeer.com/FindPlace_Pop.asp", Arrays.asList(new BasicNameValuePair("PlaceName",
								findPlacesCommand.getQuery()))));
			...
			case AddAvailability:
				...
				List<BasicNameValuePair> params = new ArrayList<BasicNameValuePair>(Arrays.asList(
						...
						new BasicNameValuePair("PlaceName",
								addAvCommand.getExtraPlaceId() == AddAvailabilityCommand.NO_EXTRA_PLACE ? ""
										: addAvCommand.getExtraPlaceName()), new BasicNameValuePair("PlaceName2",
								addAvCommand.getExtraPlaceId() == AddAvailabilityCommand.NO_EXTRA_PLACE ? ""
										: addAvCommand.getExtraPlaceName()), new BasicNameValuePair("PlaceID",
				...
				for (int p = 0; p < addAvCommand.getSelectedFavourites().length; p++) {
					params.add(new BasicNameValuePair("placeid", Integer
							.toString(addAvCommand.getSelectedFavourites()[p])));
				}
			...
			case AddToCellar:
				...
				if (addToCellarCommand.isWant()) {
					HttpHelper.makeRBPost("http://www.ratebeer.com/beerlistwant-process.asp", 
						Arrays.asList(
								new BasicNameValuePair("BeerID", Integer.toString(addToCellarCommand.getBeerId())),
								new BasicNameValuePair("memo", addToCellarCommand.getMemo()),
								new BasicNameValuePair("submit", "Add")));
				} else {
					HttpHelper.makeRBPost("http://www.ratebeer.com/beerlisthave-process.asp", 
						Arrays.asList(
								new BasicNameValuePair("BeerID", Integer.toString(addToCellarCommand.getBeerId())),
								new BasicNameValuePair("Update", "0"),
								new BasicNameValuePair("vintage", addToCellarCommand.getVintage()),
								new BasicNameValuePair("memo", addToCellarCommand.getMemo()),
								new BasicNameValuePair("quantity", addToCellarCommand.getQuantity()),
								new BasicNameValuePair("submit", "Add")));
				}
			...
			return new CommandFailureResult(command, new ApiException(ApiException.ExceptionType.CommandFailed, e
					.toString()));
		}
	}
	...
	private CommandResult parseUserId(GetUserIdCommand command, String response) {
		...
		/*String drinkingText = "TODO";
		int drinkingStart = response.indexOf(drinkingText);
		int drinkingEnd = response.indexOf("TODO", drinkingStart + drinkingText.length());*/
		String drinkingStatus = "";
		/*if (drinkingStart >= 0 && drinkingEnd > 0) {
			drinkingStatus = response.substring(drinkingStart + drinkingText.length(), drinkingEnd);
			int linkEnd = drinkingStatus.indexOf(">") + 1;
			if (linkEnd > 0 && drinkingStatus.indexOf("<", linkEnd) > 0) {
				// Strip the 'a href' tag from this drinking status, leaving only the actual beer name
				drinkingStatus = drinkingStatus.substring(linkEnd, drinkingStatus.indexOf("<", linkEnd));
			}
			drinkingStatus = cleanHtml(drinkingStatus);
		}*/
	...
	private CommandResult parseUserDetails(GetUserDetailsCommand command, String html) throws ParseException {
		...
		int beerRateCount = Integer.parseInt(html.substring(beerRateCountStart, html
				.indexOf("</b>", beerRateCountStart)));

		int placeRateCountStart = html.indexOf("<b>", beerRateCountStart) + "<b>".length();
		int placeRateCount = Integer.parseInt(html.substring(placeRateCountStart, html.indexOf("</b>",
				placeRateCountStart)));
		...
		if (avgScoreGivenPresent >= 0) {
			...
			avgScoreGiven = cleanHtml(html.substring(avgScoreGivenStart, html.indexOf(" ", avgScoreGivenStart)));
		}
		...
		if (avgBeerRatedPresent >= 0) {
			...
			avgBeerRated = cleanHtml(html.substring(avgBeerRatedStart, html.indexOf("<", avgBeerRatedStart)));
		}

		String styleText = "Favorite style: <a href=\"/beerstyles/";
		int styleStart = html.indexOf(styleText, placeRateCountStart) + styleText.length();
		int styleIdStart = html.indexOf("/", styleStart) + 1;
		int styleId = Integer.parseInt(html.substring(styleIdStart, html.indexOf("/", styleIdStart)));

		int styleNameStart = html.indexOf("<b>", styleIdStart) + "<b>".length();
		String styleName = cleanHtml(html.substring(styleNameStart, html.indexOf("</b>", styleNameStart)));

		List<RecentBeerRating> ratings = new ArrayList<RecentBeerRating>();
		String ratingText = "style=\"height: 21px;\"><A HREF=\"/beer/";
		int ratingStart = html.indexOf(ratingText, styleNameStart);
		while (ratingStart >= 0) {
			...
			String beerName = cleanHtml(html.substring(beerNameStart, html.indexOf("<", beerNameStart)));

			int beerStyleStart = html.indexOf("smallGray\">", beerNameStart) + "smallGray\">".length();
			String beerStyle = cleanHtml(html.substring(beerStyleStart, html.indexOf("<", beerStyleStart)));
	...
	private CommandResult parseUserRatings(GetUserRatingsCommand command, String html) throws ParseException {
		...
		while (rowStart > 0 + rowText.length()) {
			...
			String beerName = cleanHtml(html.substring(beerStart, html.indexOf("<", beerStart)));

			int brewerStart = html.indexOf("/\">", beerStart) + 3;
			String brewerName = cleanHtml(html.substring(brewerStart, html.indexOf("<", brewerStart)));

			int styleStart = html.indexOf("\">", brewerStart) + "\">".length();
			String styleName = cleanHtml(html.substring(styleStart, html.indexOf("<", styleStart)));
	...
	private CommandResult parseUserRating(GetUserRatingCommand command, String html) throws ParseException {
		...
		String comments = cleanHtml(html.substring(commentsStart, html.indexOf("<", commentsStart)));
	...
	private CommandResult parseUserCellar(GetUserCellarCommand command, String html) throws ParseException {
		...
		while (wantRowStart > 0 + wantRowText.length() && wantRowStart < havesStart) {
			...
			String memo = cleanHtml(html.substring(memoStart, html.indexOf("<", memoStart))).trim();
		...
		while (haveRowStart > 0 + haveRowText.length()) {
			...
			int quantityStart = html.indexOf("valign=bottom align=center>", beerStart) + "valign=bottom align=center>".length();
			String quantity = cleanHtml(html.substring(quantityStart, html.indexOf("</td>", quantityStart))).trim();

			int memoStart = html.indexOf("beerfoot valign=top>", quantityStart) + "beerfoot valign=top>".length();
			String memo = cleanHtml(html.substring(memoStart, html.indexOf("<br><span", memoStart))).trim();

			int vintageStart = html.indexOf("class=beerfoot><i>", beerStart) + "class=beerfoot><i>".length();
			String vintage = cleanHtml(html.substring(vintageStart, html.indexOf("</i>", vintageStart))).trim();
	...
	private CommandResult parseStyleBeers(GetStyleDetailsCommand command, String html) {
		...
		int headerStart = html.indexOf("<TD class=\"beer\" valign=\"top\">");
		int nameStart = html.indexOf("<h1>", headerStart) + "<h1>".length();
		String name = cleanHtml(html.substring(nameStart, html.indexOf("</h1>", nameStart)));
		
		int descriptionStart = html.indexOf("100%;\">", nameStart) + "100%;\">".length();
		String description = cleanHtml(html.substring(descriptionStart, html.indexOf("</div>", descriptionStart)).trim());
		...
		while (servedInStart >= 0) {
			servedIn.add(cleanHtml(html.substring(servedInStart, html.indexOf("<br>", servedInStart))));
	...
	private CommandResult parseEvents(GetEventsCommand command, String html) {
		...
		while (rowStart > 0 + rowText.length()) {
			...
			String event = cleanHtml(html.substring(nameStart, html.indexOf("<", nameStart)));
	...
	private CommandResult parseEventDetails(GetEventDetailsCommand command, String html) throws ParseException {
		...
		String location = cleanHtml(html.substring(locationStart, html.indexOf("<br>", locationStart)));

		int addressStart = html.indexOf("\">", locationStart) + "\">".length();
		String address = cleanHtml(html.substring(addressStart, html.indexOf(" [ map ]", addressStart)).trim());
		...
		String details = cleanHtml(html.substring(detailsStart, html.indexOf("<b>Cost:</b>", detailsStart))).trim();

		String contactText = "<h3>Contact Info</h3><br>";
		int contactStart = html.indexOf(contactText, detailsStart) + contactText.length();
		String contact = cleanHtml(html.substring(contactStart, html.indexOf("</i>", contactStart))).trim();
	...
	private CommandResult parseFavouritePlaces(GetFavouritePlacesCommand command, String html) throws ParseException {
		...
		while (rowStart > 0 + rowText.length()) {
			...
			String placeName = cleanHtml(html.substring(placeNameStart, html.indexOf("<", placeNameStart)));
	...
	private CommandResult parseSearchResults(SearchCommand command, String html) {
		...
		try {
			...
			for (int i = 0; i < json.length(); i++) {
				...
				results.add(new SearchResult(Integer.parseInt(result.getString("BeerID")), cleanHtml(result
						.getString("BeerName")),
		...
		try {
			...
			for (int i = 0; i < json.length(); i++) {
				...
				results.add(new BeerRating(result.getString("resultNum"), result.getString("RatingID"), result
						...
						cleanHtml(result.getString("Comments")), timeEntered, timeUpdated, result.getString("UserID"),
						cleanHtml(result.getString("UserName")), cleanHtml(result.getString("City")), result
								.getString("StateID"), cleanHtml(result.getString("State")), result
								.getString("CountryID"), cleanHtml(result.getString("Country")), result
								.getString("RateCount")));
			}
	...
	private CommandResult parsePlaces(GetPlacesCommand command, String html) {
		...
		try {
			...
			for (int i = 0; i < json.length(); i++) {
				...
				results.add(new Place(result.getInt("PlaceID"), cleanHtml(result.getString("PlaceName")), result
						.getInt("PlaceType"), cleanHtml(result.getString("Address")), cleanHtml(result
						.getString("City")), result.getString("StateID"), result.getInt("CountryID"), cleanHtml(result
						.getString("PostalCode")), cleanHtml(result.getString("PhoneNumber")),
						avgRating.equals("null") ? -1 : (int) Float.parseFloat(avgRating), cleanHtml(result
	...
	private CommandResult parseBeerDetails(GetBeerDetailsCommand command, String html) throws ApiException {
		...
		try {
			...
			if (json.length() > 0) {
				...
				details = new BeerDetails(result.getInt("BeerID"), cleanHtml(result.getString("BeerName")), result
						.getInt("BrewerID"), cleanHtml(result.getString("BrewerName")), cleanHtml(result
						.getString("BeerStyleName")), (float) result.getDouble("Alcohol"),
						overall.equals("null") ? GetBeerDetailsCommand.NO_SCORE_YET : Float.parseFloat(overall), style
								.equals("null") ? GetBeerDetailsCommand.NO_SCORE_YET : Float.parseFloat(style),
						cleanHtml(result.getString("Description")));
			}
