...
public class HomeTablet extends RateBeerActivity {
	...
	private static final int TAB_BEERS = 1;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// Add tab-like navigation to action bar
		/*bar.setNavigationMode(ActionBar.NAVIGATION_MODE_TABS);
		bar.addTab(bar.newTab().setIcon(android.R.drawable.ic_menu_slideshow).setTabListener(onTabSelected));
		bar.addTab(bar.newTab().setIcon(android.R.drawable.ic_menu_sort_by_size).setTabListener(onTabSelected));
		bar.addTab(bar.newTab().setIcon(android.R.drawable.ic_menu_mylocation).setTabListener(onTabSelected));
		bar.addTab(bar.newTab().setIcon(android.R.drawable.ic_menu_today).setTabListener(onTabSelected));
        if(savedInstanceState != null) {
        	bar.setSelectedNavigationItem(savedInstanceState.getInt(STATE_SELECTEDTAB));
        }*/
        showSearch();
	...
	protected void onSaveInstanceState(Bundle outState) {
		/*outState.putInt(STATE_SELECTEDTAB, getSupportActionBar().getSelectedNavigationIndex());*/
		super.onSaveInstanceState(outState);
	}

	/*private TabListener onTabSelected = new TabListener() {
		@Override
		...
		public void onTabSelected(Tab tab, FragmentTransaction ft) {
			...
			case TAB_DASHBOARD:
				break;
			case TAB_BEERS:
				// TODO: Disable this tab or something when no user is signed in
				if (getUser() != null)	{
					load(new UserViewFragment(getUser().getUsername(), getUser().getUserID()));
				}
				break;
			case TAB_PLACES:
				load(new PlacesFragment());
				break;
			case TAB_EVENTS:
				load(new EventsFragment());
				break;
			}
		}
		@Override
