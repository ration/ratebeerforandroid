...
public class PosterService extends RateBeerService {
	...
	protected void onHandleIntent(Intent intent) {
		...
		if (settings.getUserSettings() == null) {
			Log.d(RateBeerForAndroid.LOG_NAME, "Canceling " + intent.getAction() + " intent because there are no user settings known.");
			return;
		}
		...
		if (intent.getAction().equals(ACTION_SETDRINKINGSTATUS)) {
			...
			if (beerId == NO_BEER_EXTRA) {
				// If no specific beer was tight to this drinking status, assume it was from the home screen's free text input
				recoverIntent = new Intent(this, Home.class);
			} else {
				recoverIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(String.format(URI_BEER, Integer.toString(beerId))));
			}
			createNotification(NOTIFY_SETDRINKINGSTATUS, getString(R.string.app_settingdrinking), getString(R.string.home_nowdrinking, newStatus), 
					true, recoverIntent);
			...
			if (result instanceof CommandSuccessResult) {
				...
				String e = result instanceof CommandFailureResult? ((CommandFailureResult)result).getException().toString(): "Unknown error";
		...
		if (intent.getAction().equals(ACTION_POSTRATING)) {
			...
			if (beerId == NO_BEER_EXTRA || aroma <= 0 || appearance <= 0 || taste <= 0 || palate <= 0 || overall <= 0 || beerName == null || 
					comment == null) {
			...
			createNotification(NOTIFY_POSTINGRATING, getString(R.string.app_postingrating), 
					getString(R.string.app_rated, beerName, PostRatingCommand.calculateTotal(aroma, appearance, taste, palate, overall)), 
					true, recoverIntent);
			CommandResult result = new PostRatingCommand(app.getApi(), beerId, ratingId, origDate, beerName, aroma, appearance, taste, palate, overall, 
					comment).execute();
			if (result instanceof CommandSuccessResult) {
		...
		if (intent.getAction().equals(ACTION_ADDAVAILABILITY)) {
			...
			Intent recoverIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(String.format(URI_BEER, Integer.toString(beerId))));
			createNotification(NOTIFY_ADDAVAILABILITY, getString(R.string.app_addingavailability), getString(R.string.app_addingforbeer, beerName), 
					true, recoverIntent);
			CommandResult result = new AddAvailabilityCommand(app.getApi(), beerId, selectedPlaces, extraPlaceName, extraPlaceId, isOnBottleCan, isOnTap).execute();
			if (result instanceof CommandSuccessResult) {
		...
		if (intent.getAction().equals(ACTION_ADDTOCELLAR)) {
			...
			createNotification(NOTIFY_ADDTOCELLAR, getString(R.string.app_addingtocellar), 
					...
					true, recoverIntent);
			CommandResult result = new AddToCellarCommand(app.getApi(), cellarType, beerId, memo, vintage, quantity).execute();
			if (result instanceof CommandSuccessResult) {
		...
		if (intent.getAction().equals(ACTION_SENDMAIL)) {
			...
			createNotification(NOTIFY_SENDMAIL, getString(R.string.mail_sendingmail), 
					getString(R.string.mail_sendingto, sendTo), true, recoverIntent);
	...
	private void createNotification(int id, String line1, String line2, boolean autoCancel, Intent contentIntent) {
		...
		notification.setLatestEventInfo(getApplicationContext(), line1, line2, PendingIntent.getActivity(this, 0, contentIntent, 0));
