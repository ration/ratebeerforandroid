...
public class UserRatingsFragment extends RateBeerFragment {
	...
	public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
		MenuItem item = menu.add(Menu.NONE, RateBeerActivity.MENU_REFRESH, RateBeerActivity.MENU_REFRESH, R.string.app_refresh);
	...
	public boolean onOptionsItemSelected(android.view.MenuItem item) {
		...
		case RateBeerActivity.MENU_REFRESH:
			...
			break;
		case MENU_SORTBY:
			new UserRatingsSortDialog().show(getActivity().getSupportFragmentManager(), null);
			break;
		}
	...
	private OnItemClickListener onItemSelected = new OnItemClickListener() {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
			UserRating item = ((UserRatingsAdapter) ((android.widget.HeaderViewListAdapter)ratingsView.getAdapter()).getWrappedAdapter()).getItem(position);
	...
	private void publishResults(ArrayList<UserRating> result) {
		...
		if (ratingsView.getAdapter() == null || (ratingsView.getAdapter() instanceof HeaderViewListAdapter && 
				((UserRatingsAdapter) ((android.widget.HeaderViewListAdapter)ratingsView.getAdapter()).getWrappedAdapter()) == null)) {
			ratingsView.setAdapter(new UserRatingsAdapter(getActivity(), result));
		} else {
			((UserRatingsAdapter) ((android.widget.HeaderViewListAdapter)ratingsView.getAdapter()).getWrappedAdapter()).addAll(result);
		}
	...
	protected class UserRatingsSortDialog extends DialogFragment {
	    ...
	    public Dialog onCreateDialog(Bundle savedInstanceState) {
	    	return new AlertDialog.Builder(getActivity())
	    		.setTitle(R.string.myratings_sortby)
	    		.setItems(R.array.myratings_sortby_names, new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int which) {
						...
						// GetUserRatingsCommand's order ID, f.e. the fourth array item 'My rating' is the sort order 
						// with ID 5 (which is SORTBY_DATE)
